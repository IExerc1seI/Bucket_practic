...
A multi-agent geospatial data collection system that uses a **local LLM (LLaMA 3 via Ollama)** to decompose high-level collection tasks into protocol-specific subtasks, execute them in parallel, validate results, and store raw data with metadata in an S3-compatible storage (MinIO).
...
Open-source LLMs (LLaMA 3) may still produce unstable output format in edge cases
Requires prompt engineering for strict JSON output
Some external APIs may return HTML instead of JSON
Large data streaming may require tuning