...
A multi-agent geospatial data collection system that uses a **local LLM (Gemma via Ollama)** to decompose high-level collection tasks into protocol-specific subtasks, execute them in parallel, validate results, and store raw data with metadata in an S3-compatible storage (MinIO).
...
Open-source LLMs (Gemma) may produce unstable output format
